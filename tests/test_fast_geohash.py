...
def test_encode_explicit_len() -> None:
	result = fast_geohash.encode((37.8324, 112.5584), precision=9)

	assert result == 'ww8p1r4t8'


def test_encode_default_len() -> None:
	result = fast_geohash.encode((37.8324, 112.5584))
...
def test_encode__invalid_coordinates() -> None:
	with pytest.raises(ValueError) as e:
		fast_geohash.encode((float('nan'), 112.5584), 9)
...
def test_encode__invalid_length() -> None:
	with pytest.raises(ValueError) as e:
		fast_geohash.encode((37.8324, 112.5584), 14)
...
def test_decode() -> None:
	assert fast_geohash.decode('ww8p1r4t8') == ('37.8324', '112.5584')
